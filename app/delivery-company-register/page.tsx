    ...
    return (
      <main className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="max-w-md w-full rounded-3xl border border-emerald-200 bg-white p-8 text-center shadow-sm">
          ...
          <Link href="/" className="mt-6 inline-block rounded-xl bg-slate-900 px-5 py-3 text-sm font-bold text-white">Back to YOMICO</Link>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <header className="bg-gradient-to-r from-slate-900 to-slate-800 text-white">
        <div className="max-w-5xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          ...
          <Link href="/delivery-company/login" className="text-sm font-semibold hover:underline">Company already approved? Sign in →</Link>
        </div>
      </header>

      <section className="bg-gradient-to-r from-slate-900 via-slate-800 to-teal-800 text-white">
        <div className="max-w-5xl mx-auto px-5 sm:px-8 pb-10 pt-4">
          ...
          <h1 className="mt-4 text-3xl md:text-5xl font-black tracking-tight">Partner as a delivery company</h1>
      ...
      <section className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <div className="h-2.5 rounded-full bg-slate-100 overflow-hidden">
            <div className="h-full rounded-full bg-gradient-to-r from-slate-700 to-teal-500 transition-all duration-500" style={{ width: `${progress}%` }} />
          </div>
        </div>
        ...
        <Section number="03" title="Service Areas" description="Where your company can deliver (comma separated).">
          ...
          {areas.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {areas.map((a) => <span key={a} className="rounded-full bg-teal-50 border border-teal-200 px-3 py-1 text-xs text-teal-800">{a}</span>)}
            </div>
          )}
        </Section>
        ...
        <Section number="05" title="Authorised Person & Login" description="The person who will manage the company console. Their email + password is the login.">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-2">Minimum 6 characters. Used to sign in to the Delivery Company Console once approved.</p>
            </div>
          </div>
        </Section>
        ...
        <Section number="07" title="Declaration & Submit" description="Confirm and submit your application for review.">
          ...
          <button
            ...
            className={`mt-5 w-full rounded-2xl py-4 text-lg font-black text-white shadow-lg transition ${
              loading ? "bg-slate-400 cursor-not-allowed" : "bg-gradient-to-r from-slate-900 to-teal-700 hover:-translate-y-0.5"
            }`}
          >
...
/* ---------------- Reusable presentational components (local, vendor-register style) ---------------- */
  ...
  return (
    <section className="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 md:p-8">
      <div className="mb-6 flex items-start gap-4">
        <div className="w-10 h-10 shrink-0 rounded-2xl bg-gradient-to-br from-slate-800 to-teal-600 text-white flex items-center justify-center font-black shadow-md">{number}</div>
        <div>
