...
export default function DeliveryPartnersPage() {
  ...
  const loadPartners = async () => {
    try {
      const snapshot = await getDocs(collection(db, "deliveryPartners"));
  ...
  const addPartner = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "deliveryPartners"), {
        name,
        phone,
        email,
        vehicleNumber,
  ...
  const filteredPartners = partners.filter(
    (partner) =>
      ...
      (partner.serviceArea || "").toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {filteredPartners.map((partner) => (
              <div
                key={partner.id}
                className="bg-white rounded-3xl shadow-lg hover:shadow-2xl hover:-translate-y-1 transition duration-300 p-6 flex justify-between items-start gap-4"
              >
                <div>
                ...
                <div className="flex flex-col gap-3 shrink-0">
                  ...
                  <button onClick={() => deletePartner(partner.id)} className="bg-red-600 hover:bg-red-700 transition text-white px-5 py-2 rounded-xl">
                    Delete
                  </button>
                </div>
              </div>
      ...
      <div className="text-center py-8 text-gray-500">
        Manage your delivery fleet from one dashboard.
      </div>
    </div>
  );
}
